...
Dependency split rules:
  - build/package*.xml dependencies install into the Ubuntu build container
  - build/cross_deps.json can override package system_depend arch/cross_scope
  - cross_scope="host" installs into Ubuntu; cross_scope="sysroot" installs into the Bianbu sysroot
  - system_depend with arch="x86_64" installs into Ubuntu by default
...
Environment overrides:
  ...
  SROBOTIS_CROSS_HOST_PREFIX       Default: <cross-output>/host
...
ensure_bianbu_image() {
  ...
  if [[ "${image}" != "bianbu:${tag}" && "${image}" != "${remote_image}" ]]; then
    echo "[cross] Pulling ${image} ..."
    docker pull "${image}"
    return 0
  fi
...
cross_required_mounts() {
  ...
  if [[ -n "${CROSS_HOST_PREFIX:-}" ]] && \
    ! path_is_or_under "${REPO_ROOT}" "${CROSS_HOST_PREFIX}" && \
    ! path_is_or_under "${CROSS_OUTPUT_ROOT:-}" "${CROSS_HOST_PREFIX}"; then
    printf '%s\n' "${CROSS_HOST_PREFIX}"
  fi
}
...
read_cross_overlay_sysdeps() {
  local pkg_key="$1"
  local cross_deps_json="${REPO_ROOT}/build/cross_deps.json"
  [[ -f "${cross_deps_json}" ]] || return 0

  if ! has_jq; then
    echo "[cross] ERROR: jq is required to parse build/cross_deps.json." >&2
    return 1
  fi

  jq -r --arg pkg "${pkg_key}" --arg sep "${CROSS_DEP_FIELD_SEP}" '
    def dep_rows($dep_type):
      (.packages[$pkg].system_dependencies[$dep_type][]? |
        [$pkg, $dep_type, (.name // ""), (.check // ""), (.arch // ""), (.cross_scope // "")] |
        join($sep)
      );
    dep_rows("required"), dep_rows("optional")
  ' "${cross_deps_json}"
}

read_cross_package_base_sysdeps() {
  local pkg_key="$1"
  local pkg_xml
...
read_cross_package_sysdeps() {
  local pkg_key="$1"
  local deps_output
  if ! deps_output="$({ read_cross_package_base_sysdeps "${pkg_key}"; read_cross_overlay_sysdeps "${pkg_key}"; })"; then
    return 1
  fi

  local dep_line dep_type dep_name check_cmd arch_filter cross_scope dep_key
  local -a dep_order=()
  local -A dep_seen=()
  local -A dep_types=()
  local -A dep_checks=()
  ...
  while IFS= read -r dep_line; do
    [[ -n "${dep_line}" ]] || continue
    IFS="${CROSS_DEP_FIELD_SEP}" read -r _pkg_key dep_type dep_name check_cmd arch_filter cross_scope <<< "${dep_line}"
    [[ -n "${dep_name}" ]] || continue

    dep_key="${dep_name}"
    if [[ -z "${dep_seen[${dep_key}]:-}" ]]; then
      dep_order+=("${dep_key}")
      dep_seen["${dep_key}"]=1
    fi

    [[ -n "${dep_type}" ]] && dep_types["${dep_key}"]="${dep_type}"
    [[ -n "${check_cmd}" ]] && dep_checks["${dep_key}"]="${check_cmd}"
    [[ -n "${arch_filter}" ]] && dep_arches["${dep_key}"]="${arch_filter}"
    [[ -n "${cross_scope}" ]] && dep_scopes["${dep_key}"]="${cross_scope}"
  done <<< "${deps_output}"

  for dep_key in "${dep_order[@]}"; do
    dep_type="${dep_types[${dep_key}]:-required}"
    check_cmd="${dep_checks[${dep_key}]:-dpkg -s ${dep_key}}"
    arch_filter="${dep_arches[${dep_key}]:-}"
    cross_scope="${dep_scopes[${dep_key}]:-}"
    printf "%s%s%s%s%s%s%s%s%s%s\n" \
      "${pkg_key}" "${CROSS_DEP_FIELD_SEP}" \
      "${dep_type}" "${CROSS_DEP_FIELD_SEP}" \
      "${dep_key}" "${CROSS_DEP_FIELD_SEP}" \
      "${check_cmd}" "${CROSS_DEP_FIELD_SEP}" \
      "${arch_filter}" "${CROSS_DEP_FIELD_SEP}${cross_scope}"
  done
}
  ...
  case "${cmd}" in
    ...
    package|pkg)
      ...
      while IFS= read -r selected_pkg; do
        ...
        read_cross_package_sysdeps "${selected_pkg}"
      done < <(cross_selected_package_keys "${cmd}" "$@")
      ;;
...
write_cross_sysroot_deps_stamp_if_synced() {
  local fingerprint="$1"
  local deps_changed="${2:-0}"

  if [[ "${SROBOTIS_CROSS_SKIP_SYSROOT_SYNC:-0}" == "1" ]]; then
    if [[ "${deps_changed}" == "1" ]]; then
      echo "[cross] Sysroot sync skipped; not updating dependency stamp"
    fi
    return 0
  fi

  write_cross_sysroot_deps_stamp "${fingerprint}"
}
...
write_meson_cross_file() {
  CROSS_MESON_CROSS_FILE="$(cross_meson_cross_file)"
  mkdir -p "$(dirname "${CROSS_MESON_CROSS_FILE}")"

  cat > "${CROSS_MESON_CROSS_FILE}" <<MESON_CROSS
[binaries]
...
pkg_config_libdir = ['${CROSS_STAGING_PREFIX}/lib/pkgconfig', '${CROSS_STAGING_PREFIX}/share/pkgconfig', '${CROSS_SYSROOT}/usr/lib/riscv64-linux-gnu/pkgconfig', '${CROSS_SYSROOT}/usr/lib/pkgconfig', '${CROSS_SYSROOT}/usr/share/pkgconfig']
c_args = ['--sysroot=${CROSS_SYSROOT}', '-mstrict-align', '-fno-aggressive-loop-optimizations']
cpp_args = ['--sysroot=${CROSS_SYSROOT}', '-mstrict-align', '-fno-aggressive-loop-optimizations']
...
cross_cargo_home() {
  if [[ -n "${SROBOTIS_CROSS_CARGO_HOME:-}" ]]; then
    printf "%s\n" "${SROBOTIS_CROSS_CARGO_HOME}"
  elif [[ -n "${CROSS_OUTPUT_ROOT:-}" ]]; then
    printf "%s\n" "${CROSS_OUTPUT_ROOT}/.cargo"
  else
    printf "%s\n" "${DEFAULT_OUTPUT_ROOT}/cross/${BUILD_TARGET:-target}/.cargo"
  fi
}

cross_host_prefix() {
  if [[ -n "${SROBOTIS_CROSS_HOST_PREFIX:-}" ]]; then
    printf "%s\n" "${SROBOTIS_CROSS_HOST_PREFIX}"
  elif [[ -n "${CROSS_HOST_PREFIX:-}" ]]; then
    printf "%s\n" "${CROSS_HOST_PREFIX}"
  elif [[ -n "${CROSS_OUTPUT_ROOT:-}" ]]; then
    printf "%s\n" "${CROSS_OUTPUT_ROOT}/host"
  else
    printf "%s\n" "${DEFAULT_OUTPUT_ROOT}/cross/${BUILD_TARGET:-target}/host"
  fi
}

cross_meson_cross_file() {
  if [[ -n "${SROBOTIS_MESON_CROSS_FILE:-}" ]]; then
    printf "%s\n" "${SROBOTIS_MESON_CROSS_FILE}"
  elif [[ -n "${CROSS_MESON_CROSS_FILE:-}" ]]; then
    printf "%s\n" "${CROSS_MESON_CROSS_FILE}"
  elif [[ -n "${CROSS_OUTPUT_ROOT:-}" ]]; then
    printf "%s\n" "${CROSS_OUTPUT_ROOT}/toolchain/riscv64-linux-gnu.meson"
  else
    printf "%s\n" "${DEFAULT_OUTPUT_ROOT}/cross/${BUILD_TARGET:-target}/toolchain/riscv64-linux-gnu.meson"
  fi
}

detect_cross_rust_target() {
  if [[ -n "${SROBOTIS_CARGO_TARGET:-}" ]]; then
    printf "%s\n" "${SROBOTIS_CARGO_TARGET}"
    return 0
  fi

  local candidate
  while IFS= read -r candidate; do
    if compgen -G "${candidate}/lib/libcore-*.rlib" >/dev/null; then
      basename "${candidate}"
      return 0
    fi
...
detect_cross_rust_sysroot() {
  if [[ -n "${SROBOTIS_RUST_SYSROOT:-}" ]]; then
    printf "%s\n" "${SROBOTIS_RUST_SYSROOT}"
    return 0
  fi
...
configure_cross_output_paths() {
  ...
  CROSS_MESON_CROSS_FILE="${SROBOTIS_MESON_CROSS_FILE:-${CROSS_OUTPUT_ROOT}/toolchain/riscv64-linux-gnu.meson}"
  CROSS_STAGING_PREFIX="${CROSS_OUTPUT_ROOT}/staging"
  CROSS_ROOTFS_PREFIX="${CROSS_OUTPUT_ROOT}/rootfs"
}

run_build_locally_with_cross_output() {
  env \
    ...
    "SROBOTIS_OUTPUT_ROOT=${CROSS_OUTPUT_ROOT}" \
    "SROBOTIS_OUTPUT_STAGING=${CROSS_STAGING_PREFIX}" \
    "SROBOTIS_OUTPUT_ROOTFS=${CROSS_ROOTFS_PREFIX}" \
    "SROBOTIS_OUTPUT=${CROSS_STAGING_PREFIX}" \
    "SDK_BUILD_ARCH=riscv64" \
    "SROBOTIS_CROSS_BUILD=1" \
    "SROBOTIS_SKIP_DEPS_CHECK=1" \
    "${REPO_ROOT}/build/build.sh" "$@"
}

cross_should_deploy_rootfs_after_build() {
  local cmd="$1"
  shift || true
...
cross_cmake_extra_args() {
  local rust_target rust_sysroot cargo_home host_prefix
  rust_target="${CROSS_RUST_TARGET:-}"
  if [[ -z "${rust_target}" ]]; then
    rust_target="$(detect_cross_rust_target)"
  fi
  rust_sysroot="${CROSS_RUST_SYSROOT:-}"
  if [[ -z "${rust_sysroot}" ]]; then
    rust_sysroot="$(detect_cross_rust_sysroot)"
  fi
  cargo_home="$(cross_cargo_home)"
  host_prefix="$(cross_host_prefix)"
  ...
  printf '%s\n' \
    ...
    "-DCMAKE_PROGRAM_PATH=${host_prefix}/bin" \
    "-DCMAKE_LIBRARY_PATH=${host_prefix}/lib;${host_prefix}/lib/x86_64-linux-gnu" \
    "-DCARGO_EXECUTABLE=/usr/bin/cargo" \
    "-DSROBOTIS_CARGO_HOME=${cargo_home}" \
    "-DSROBOTIS_CARGO_RUSTC=/usr/bin/rustc" \
...
cross_target_cache_root() {
  printf "%s\n" "${DEFAULT_OUTPUT_ROOT}/cross/.cache/${CROSS_BIANBU_TAG:-target}"
}

cross_selected_package_keys() {
  local cmd="$1"
  shift || true
  ...
  case "${cmd}" in
    all|cmake|C|ros2|R)
      has_jq || { echo "[cross] ERROR: jq is required for target JSON parsing." >&2; return 1; }
      local pkg_key
      while IFS= read -r pkg_key; do
        [[ -n "${pkg_key}" ]] || continue
        case "${cmd}" in
          cmake|C)
            [[ "${pkg_key}" == middleware/ros2/* || "${pkg_key}" == application/ros2/* ]] && continue
            ;;
          ros2|R)
            [[ "${pkg_key}" == middleware/ros2/* || "${pkg_key}" == application/ros2/* ]] || continue
            ;;
...
" "${pkg_key}"
    ...
    package|pkg)
      ...
      local action="${2:-build}"
      [[ -n "${pkg_arg}" ]] || return 0
      [[ "${action}" == "clean" ]] && return 0

      local pkg_key
      pkg_key="$(package_key_from_arg "${pkg_arg}")"
      case "${action}" in
        --with-deps)
          if package_is_ros2 "${pkg_arg}"; then
            local sdk_dep closure
            while IFS= read -r sdk_dep; do
              [[ -n "${sdk_dep}" ]] || continue
              closure="$(resolve_nonros2_dependency_closure "${sdk_dep}" 1)"
              [[ -n "${closure}" ]] && printf "%s
" "${closure}"
...
" "${pkg_key}"
            ...
            resolve_nonros2_dependency_closure "${pkg_key}" 1
          fi
          ;;
        --deps)
          if package_is_ros2 "${pkg_arg}"; then
...
" "${closure}"
            ...
            resolve_nonros2_dependency_closure "${pkg_key}" 0
          fi
          ;;
        *)
          printf "%s
...
cross_prebuild_hook_entries() {
  ...
  while IFS= read -r pkg_key; do
    [[ -n "${pkg_key}" ]] || continue
    if hook="$(cross_package_hook_path "${pkg_key}" "${hook_name}")"; then
      printf "%s|%s
" "${pkg_key}" "${hook}"
    fi
  done < <(cross_selected_package_keys "$@")
}

cross_host_prebuild_hook_entries() {
  cross_prebuild_hook_entries "cross_prebuild_host.sh" "$@"
}

cross_target_prebuild_hook_entries() {
  cross_prebuild_hook_entries "cross_prebuild_target.sh" "$@"
}
prepare_cross_owned_paths_in_container() {
  local container_name="$1"
  local uid="$2"
  local gid="$3"
  shift 3 || true
  ...
  docker exec -i --workdir / --user "0:0" "${container_name}" \
    bash -c "set -euo pipefail; uid=\"\$1\"; gid=\"\$2\"; shift 2; for path in \"\$@\"; do mkdir -p \"\${path}\"; chown -R \"\${uid}:\${gid}\" \"\${path}\"; done" \
    bash "${uid}" "${gid}" "${owned_paths[@]}"
}

run_host_prebuild_hooks() {
  local container_name="$1"
  local cmd="$2"
  shift 2 || true

  local hook_entries=()
  mapfile -t hook_entries < <(cross_host_prebuild_hook_entries "${cmd}" "$@")
  [[ "${#hook_entries[@]}" -gt 0 ]] || return 0

  local uid gid user_name host_home host_cargo_home cache_root host_prefix
  uid="$(id -u)"
  gid="$(id -g)"
  user_name="${USER:-builder}"
  host_home="${CROSS_OUTPUT_ROOT}/.host_home"
  host_cargo_home="$(cross_cargo_home)"
  cache_root="$(cross_target_cache_root)"
  host_prefix="$(cross_host_prefix)"

  ensure_container_user "${container_name}" "${user_name}" "${uid}" "${gid}" "${host_home}"
  prepare_cross_owned_paths_in_container "${container_name}" "${uid}" "${gid}"     "${host_home}" "${host_cargo_home}" "${cache_root}" "${host_prefix}"

  local entry pkg_key hook_path
  for entry in "${hook_entries[@]}"; do
    IFS="|" read -r pkg_key hook_path <<< "${entry}"
    [[ -n "${pkg_key}" && -n "${hook_path}" ]] || continue
    echo "[cross] Running host prebuild hook for ${pkg_key}: ${hook_path#"${REPO_ROOT}"/}"
    docker exec -i       --workdir "${REPO_ROOT}/${pkg_key}"       --user "${uid}:${gid}"       -e "HOME=${host_home}"       -e "USER=${user_name}"       -e "LOGNAME=${LOGNAME:-${user_name}}"       -e "REPO_ROOT=${REPO_ROOT}"       -e "BUILD_TARGET=${BUILD_TARGET}"       -e "BUILD_TARGET_FILE=${BUILD_TARGET_FILE}"       -e "BUILD_CONFIG_FILE=${BUILD_CONFIG_FILE}"       -e "SROBOTIS_PACKAGE_KEY=${pkg_key}"       -e "SROBOTIS_CROSS_OUTPUT_ROOT=${CROSS_OUTPUT_ROOT}"       -e "SROBOTIS_CROSS_HOST_PREFIX=${host_prefix}"       -e "SROBOTIS_CROSS_SYSROOT=${CROSS_SYSROOT}"       -e "SROBOTIS_CROSS_TOOLCHAIN_FILE=${CROSS_TOOLCHAIN_FILE}"       -e "SROBOTIS_CROSS_STAGING=${CROSS_STAGING_PREFIX}"       -e "SROBOTIS_CROSS_ROOTFS=${CROSS_ROOTFS_PREFIX}"       -e "SROBOTIS_CROSS_CACHE_ROOT=${cache_root}"       -e "SROBOTIS_CROSS_CARGO_HOME=${host_cargo_home}"       -e "SROBOTIS_CARGO_HOME=${host_cargo_home}"       -e "CARGO_HOME=${host_cargo_home}"       -e "SROBOTIS_CROSS_BIANBU_TAG=${CROSS_BIANBU_TAG}"       -e "SROBOTIS_CROSS_HOST_PREBUILD=1"       "${container_name}"       bash "${hook_path}"
  done

  chown_cross_output_in_container "${container_name}" "${uid}" "${gid}" || true
}

run_target_prebuild_hooks() {
  local container_name="$1"
  local cmd="$2"
  shift 2 || true

  local hook_entries=()
  mapfile -t hook_entries < <(cross_target_prebuild_hook_entries "${cmd}" "$@")
  [[ "${#hook_entries[@]}" -gt 0 ]] || return 0

  local uid gid user_name target_home target_cargo_home cache_root
  uid="$(id -u)"
  gid="$(id -g)"
  user_name="${USER:-builder}"
  target_home="${CROSS_OUTPUT_ROOT}/.target_home"
  target_cargo_home="${CROSS_OUTPUT_ROOT}/.cargo"
  cache_root="$(cross_target_cache_root)"

  ensure_container_user "${container_name}" "${user_name}" "${uid}" "${gid}" "${target_home}"
  prepare_cross_owned_paths_in_container "${container_name}" "${uid}" "${gid}" \
    "${target_home}" "${target_cargo_home}" "${cache_root}"

  local entry pkg_key hook_path
  for entry in "${hook_entries[@]}"; do
    IFS="|" read -r pkg_key hook_path <<< "${entry}"
    [[ -n "${pkg_key}" && -n "${hook_path}" ]] || continue
    echo "[cross] Running target prebuild hook for ${pkg_key}: ${hook_path#"${REPO_ROOT}"/}"
    docker exec -i \
      --workdir "${REPO_ROOT}/${pkg_key}" \
      --user "${uid}:${gid}" \
      -e "HOME=${target_home}" \
      -e "USER=${user_name}" \
      ...
      -e "SROBOTIS_PACKAGE_KEY=${pkg_key}" \
      ...
      -e "SROBOTIS_CROSS_CACHE_ROOT=${cache_root}" \
      -e "SROBOTIS_CROSS_CARGO_HOME=${target_cargo_home}" \
      -e "SROBOTIS_CARGO_HOME=${target_cargo_home}" \
      -e "CARGO_HOME=${target_cargo_home}" \
      -e "SROBOTIS_CROSS_BIANBU_TAG=${CROSS_BIANBU_TAG}" \
      -e "SROBOTIS_CROSS_TARGET_PREBUILD=1" \
      "${container_name}" \
      bash "${hook_path}"
  done
...
run_build_in_ubuntu() {
  ...
  local uid gid user_name cross_home target_cargo_home cache_root
  uid="$(id -u)"
  gid="$(id -g)"
  user_name="${USER:-builder}"
  cross_home="${CROSS_OUTPUT_ROOT}/.ubuntu_home"
  target_cargo_home="$(cross_cargo_home)"
  cache_root="$(cross_target_cache_root)"

  ensure_container_user "${container_name}" "${user_name}" "${uid}" "${gid}" "${cross_home}"
  prepare_cross_owned_paths_in_container "${container_name}" "${uid}" "${gid}" \
    "${cross_home}" "${target_cargo_home}" "${cache_root}"
  chown_cross_output_in_container "${container_name}" "${uid}" "${gid}" || true
  ...
  local rust_target rust_target_env rust_sysroot
  rust_target="${CROSS_RUST_TARGET:-}"
  if [[ -z "${rust_target}" ]]; then
    rust_target="$(detect_cross_rust_target)"
  fi
  rust_target_env="$(cross_cargo_target_env_name "${rust_target}")"
  rust_sysroot="${CROSS_RUST_SYSROOT:-}"
  if [[ -z "${rust_sysroot}" ]]; then
    rust_sysroot="$(detect_cross_rust_sysroot)"
  fi
  ...
  docker exec -i \
    ...
    -e "SROBOTIS_CARGO_HOME=${target_cargo_home}" \
    -e "CARGO_HOME=${target_cargo_home}" \
    -e "SROBOTIS_CARGO_RUSTC=/usr/bin/rustc" \
    -e "RUSTC=/usr/bin/rustc" \
    -e "SROBOTIS_CARGO_TARGET=${rust_target}" \
    -e "SROBOTIS_CARGO_TARGET_LINKER=/usr/bin/riscv64-linux-gnu-gcc" \
    -e "SROBOTIS_CARGO_TARGET_AR=/usr/bin/riscv64-linux-gnu-ar" \
    -e "SROBOTIS_RUST_SYSROOT=${rust_sysroot}" \
    -e "SROBOTIS_CARGO_LINK_SYSROOT=${CROSS_SYSROOT}" \
    -e "SROBOTIS_MESON_CROSS_FILE=$(cross_meson_cross_file)" \
    -e "CARGO_TARGET_${rust_target_env}_LINKER=/usr/bin/riscv64-linux-gnu-gcc" \
...
main() {
  ...
  case "${cmd}" in
    clean)
      run_build_locally_with_cross_output "${original_args[@]}"
      return $?
      ;;
  ...
  mkdir -p "${CROSS_OUTPUT_ROOT}" "$(dirname "${CROSS_SYSROOT}")" "${CROSS_HOST_PREFIX}"
  ...
  CROSS_RUST_TARGET="$(detect_cross_rust_target)"
  CROSS_RUST_SYSROOT="$(detect_cross_rust_sysroot)"
  echo "[cross] Rust cargo target: ${CROSS_RUST_TARGET} (sysroot: ${CROSS_RUST_SYSROOT})"
  write_cross_sysroot_deps_stamp_if_synced "${sysroot_deps_fingerprint}" "${sysroot_deps_changed}"
  write_toolchain_file
  write_meson_cross_file

  run_host_prebuild_hooks "${ubuntu_container}" "${cmd}" "${payload[@]}"
  run_target_prebuild_hooks "${bianbu_container}" "${cmd}" "${payload[@]}"

  run_build_in_ubuntu "${ubuntu_container}" "${original_args[@]}"
  relocate_cross_ros2_install "${CROSS_STAGING_PREFIX}"

  if cross_should_deploy_rootfs_after_build "${cmd}" "${payload[@]}"; then
    run_build_locally_with_cross_output deploy-rootfs
  fi
  relocate_cross_ros2_install "${CROSS_ROOTFS_PREFIX}"
}
